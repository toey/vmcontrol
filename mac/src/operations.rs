...
fn start_vm_with_config(smac: &str, cfg: &VmStartConfig) -> Result<String, String> {
    ...
    // Boot options (no -daemonize — mac uses spawn_cmd_with_log instead)
    qemu_args.extend([
    ...
    // Start VM as background process with stderr logging (mac-specific: no -daemonize)
...
pub fn stop(json_str: &str) -> Result<String, String> {
    ...
    // Fallback if monitor socket failed — use safe run_cmd
    if pctl_result.contains("Error") || pctl_result.contains("error") {
        output.push_str("Monitor unavailable, trying pkill...\n");
        let pattern = format!("qemu.*vncsock_{}", cmd.smac);
        let _ = run_cmd("pkill", &["-f", &pattern]);
